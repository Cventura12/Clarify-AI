  ...
  return (
    <div className="rounded-xl border border-[#e6e4e1] bg-white p-4 shadow-soft">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-[0.25em] text-slate-400">Plan Runs</p>
          <p className="text-xs text-slate-500">Recent sequential executions.</p>
        </div>
        <span className="text-xs text-slate-400">{runs.length} runs</span>
      </div>

      {runs.length === 0 ? (
        <p className="mt-2.5 text-sm text-slate-500">No plan runs yet.</p>
      ) : (
        <div className="mt-2.5 space-y-2">
          {runs.map((run) => (
            <div key={run.id} className="rounded-lg border border-[#ebe8e3] bg-[#fbfaf8] p-2.5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-semibold text-slate-800">{run.plan.task.title}</p>
                <span className="text-[11px] text-slate-400">{run.createdAt.toLocaleString()}</span>
              </div>
