              ...
              <div className={styles.requestCardBody}>
                <div className={styles.requestCardMain}>
                  <div className={styles.requestTitleRow}>
                    ...
                    <div className={styles.requestInlineTools}>
                  ...
                  <p className={styles.requestStepper}>
                    {["Interpret", "Plan", "Authorize"].map((step, stepIdx) => (
                      <span
                        key={step}
                        className={`${styles.requestStep} ${
                          stepIdx < stageIndex(item.taskStatus) ? styles.requestStepDone : ""
                        } ${stepIdx === stageIndex(item.taskStatus) ? styles.requestStepCurrent : ""}`}
                      >
                        <span className={styles.requestStepDot} />
                        <span className={styles.requestStepLabel}>{step}</span>
                        {stepIdx < 2 ? (
                          <span
                            className={`${styles.requestStepLine} ${
                              stepIdx < stageIndex(item.taskStatus) ? styles.requestStepLineDone : ""
                            }`}
                          />
                        ) : null}
                      </span>
